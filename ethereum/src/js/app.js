      ...
      .then(function (instance) {
        ...
        instance.getNumOfCandidates().then(function (numOfCandidates) {
          ...
          if (numOfCandidates == 0) {
            ...
            instance
              .addCandidate("Candidate1", "Democratic")
              .then(function (result) {
                $("#candidate-box").append(
                  `<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=0>Candidate1</label></div>`
                );
              });
            instance
              .addCandidate("Candidate2", "Republican")
              .then(function (result) {
                $("#candidate-box").append(
                  `<div class='form-check'><input class='form-check-input' type='checkbox' value='' id=${result.logs[0].args.candidateID}><label class='form-check-label' for=1>Candidate1</label></div>`
                );
              });
            // the global variable will take the value of this variable
            numOfCandidates = 2;
          } else {
            // if candidates were already added to the contract we loop through them and display them
              ...
              instance.getCandidate(i).then(function (data) {
                $("#candidate-box").append(
                  `<div class="form-check"><input class="form-check-input" type="checkbox" value="" id=${
                    ...
                    data[0]
                  }>${window.web3.toAscii(data[1])}</label></div>`
                );
              });
            }
          }
  ...
  findNumOfVotes: function () {
    VotingContract.deployed().then(function (instance) {
      ...
      for (var i = 0; i < window.numOfCandidates; i++) {
        ...
        Promise.all([candidatePromise, votesPromise])
          .then(function (data) {
